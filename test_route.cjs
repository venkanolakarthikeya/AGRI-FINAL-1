  ...
  while (fallbackIndex < fallbackModels.length) {
    ...
    while (retryCount < maxRetriesPerModel) {
      try {
        ...
        console.log(`Error on ${currentModel}: ${error.status}`);
        ...
        if (isNotFound) {
          console.warn(`[Gemini API] Model ${currentModel} not found, switching...`);
          break; // Go to next model instantly
        }

        if (isRateLimit) {
          console.warn(`[Gemini API] Rate limit (429) hit on ${currentModel}. Switching...`);
          break; // Go to next model instantly
        }
